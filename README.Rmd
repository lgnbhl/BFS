...
This could happen because you ran too many times the `bfs_get_*()` function (API config is [here](https://www.pxweb.bfs.admin.ch/api/v1/de/?config)). A solution is to wait a few seconds before running the next `bfs_get_*()`. You can for example add a delay in your R code, for example using `Sys.sleep(10)` (10 seconds delay).


#### Query specific elements

You may get an error message when running `bfs_get_*()` because the dataset is too big.
...
One solution is to query only specific elements of the dataset to download less data. 
