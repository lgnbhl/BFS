...
[Swiss Federal Statistical Office](https://www.bfs.admin.ch/bfs/en/home/statistics/catalogues-databases/data.html){target="_blank"} (BFS is for *Bundesamt für Statistik* in German) in a dynamic and reproducible way.
...
To download a BFS dataset, you have two options. You can use add the official BFS URL webpage to the `url_bfs` argument to the `bfs_get_data()`. For example, you can use the URL of a given dataset you found using `bfs_get_catalog_data()` function (see example above).
...
In case the data is not accessible using `bfs_get_catalog_data()` function, you can add manually the BFS number in the `bfs_get_data()` function using the `number_bfs` argument.
