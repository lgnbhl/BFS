...
[Swiss Federal Statistical Office](https://www.pxweb.bfs.admin.ch/pxweb/en/){target="_blank"} (BFS stands for *Bundesamt für Statistik* in German) API in a dynamic and reproducible way.
...
The `url_bfs` argument refers to the offical webpage of a dataset. Find below an example getting the `url_bfs` for a specific dataset using the R package "dplyr".
...
You can manually find the `number_bfs` by opening the official webpage and looking for the "FSO number", as showed below.
...
Under the hood, this package is using the [pxweb](https://ropengov.github.io/pxweb/index.html){target="_blank"} package to query the Swiss Federal Statistical Office PXWEB API. PXWEB is an API structure developed by Statistics Sweden and other national statistical institutions (NSI) to disseminate public statistics in a structured way.
